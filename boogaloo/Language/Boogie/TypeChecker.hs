...
checkExpression (Pos pos e) = do
  ...
  case e of
    ...
    Quantified qop tv vars triggs e -> checkQuantified qop tv vars e
...
checkStatement (Pos pos s) = do
  ...
  case s of
    ...
    Call atts lhss name args -> checkCall lhss name args
...
collectTypes (Pos pos d) = do
  modify $ setPos pos
  case d of
    TypeDecl atts ts -> mapM_ checkTypeDecl ts
    otherwise -> return ()  
...
checkTypeSynonyms (Pos pos d) = do
  modify $ setPos pos
  case d of
    TypeDecl atts ts -> mapAccum_ (locally . checkNewType) ts
    otherwise -> return ()
  where
...
checkSignatures (Pos pos d) = do
  modify $ setPos pos
  case d of
    VarDecl atts vars -> mapAccum_ (checkIdType globalScope ctxGlobals setGlobals) (map noWhere vars)
    ConstantDecl atts _ ids t _ _ -> mapAccum_ (checkIdType globalScope ctxConstants setConstants) (zip ids (repeat t))
    FunctionDecl _ name tv args ret _ -> checkFunctionSignature name tv args ret
    ProcedureDecl atts name tv args rets specs _ -> checkProcSignature name tv args rets specs
    otherwise -> return ()
...
checkBodies (Pos pos d) = do
  modify $ setPos pos
  case d of
    VarDecl atts vars -> mapAccum_ checkWhere vars
    ConstantDecl atts _ ids t (Just edges) _ -> locally $ checkParentInfo ids t (map snd edges)
    AxiomDecl atts e -> locally $ checkAxiom e
    FunctionDecl _ name tv args _ (Just body) -> locally $ checkFunction name tv args body
    ProcedureDecl atts name tv args rets specs mb -> locally $ checkProcedure tv args rets specs mb
    ImplementationDecl atts name tv args rets bodies -> locally $ checkImplementation name tv args rets bodies
    otherwise -> return ()
...
checkBody body = do
  mapAccum_ (checkIdType localScope ctxLocals setLocals) (map noWhere (concatMap snd (fst body)))
  locally $ mapAccum_ checkWhere (concatMap snd (fst body))
