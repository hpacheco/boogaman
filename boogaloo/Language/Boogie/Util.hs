...
freeVarsTwoState e = case node e of
  ...
  Quantified _ _ boundVars trggs e -> let (state, old) = freeVarsTwoState e in (state \\ map fst boundVars, old)
...
exprSubst' binding (Quantified qop tv boundVars trggs e) = Quantified qop tv boundVars trggs' (exprSubst binding' e)
  where
...
applyToQTrigAtt :: (Expression -> Expression) -> QTriggerAttribute -> QTriggerAttribute
applyToQTrigAtt f (Left trggs) = Left $ map f trggs
applyToQTrigAtt f (Right (Attribute tag values)) = Right $ Attribute tag $ map applyVal values
    where
    applyVal (EAttr e) = EAttr $ f e
    applyVal (SAttr s) = SAttr s
...
refSelections expr = case node expr of
  ...
  Quantified _ _ boundVars _ e -> refSelections e
...
removeBoundClashesAttr :: [Id] -> Attribute -> Attribute
removeBoundClashesAttr names (Attribute tag values) = Attribute tag $ map removeValues values
    where
    removeValues (EAttr e) = EAttr $ removeBoundClashes names e
    removeValues (SAttr s) = SAttr s
...
removeBoundClashes names (Pos p expr) = attachPos p $ case expr of
  ...
  Quantified op tv bv trggs e -> let
      vars = map fst bv
      types = map snd bv
      vars' = removeClashesWith vars names
      ss = M.fromList $ zip vars (map (gen . Var) vars')
      trggs' = applyToQTrigAtts (removeBoundClashes (names ++ vars') . exprSubst ss) trggs
      e' = removeBoundClashes (names ++ vars') (exprSubst ss e)
