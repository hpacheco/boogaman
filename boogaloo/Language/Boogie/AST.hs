...
type QTriggerAttribute = Either Trigger Attribute
...
data BareStatement = Predicate [Attribute] SpecClause |   -- ^ Predicate statement (assume or assert)
  ...
  Call [Attribute] [Id] Id [Expression] |                             -- ^ 'Call' @lhss proc_name args@
...
data BareDecl = 
  TypeDecl [Attribute] [NewType] |
  ConstantDecl [Attribute] Bool [Id] Type ParentInfo Bool |                                -- ^ 'ConstantDecl' @unique names type orderSpec complete@
