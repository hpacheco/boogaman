...
import Control.Monad.Trans.Except hiding (join)
...
executeProgram p tc solver recMax loopMax concretize_ solvePassing generator entryPoint = result <$> runStateT (runExceptT programExecution) (initEnv tc solver generator recMax loopMax concretize_)
  where
    programExecution = do
...
type Execution m a = ExceptT RuntimeFailure (StateT (Environment m) m) a
...
execUnsafely computation = ExceptT (Right <$> computation)
...
instance (Monad m) => Finalizer (ExceptT e m) where
  finally main cleanup = do
...
lambdaLocally expr actuals = let
    Quantified Lambda tv formals _ body = node expr
...
--  MapUpdate m args new -> evalMapUpdate m args new pos
  ...
  Quantified Lambda tv vars _ e -> evalLambda tv vars e pos
  Quantified Forall tv vars _ e -> evalForall tv vars e pos
  Quantified Exists tv vars _ e -> evalForall tv vars (enot e) pos >>= evalUnary Not
...
force val   (Pos pos (Quantified Forall tv vars _ e)) = forceForall tv vars e pos val
force val   (Pos pos (Quantified Exists tv vars _ e)) = forceForall tv vars (enot e) pos (not val)
force val   expr = do
  c <- if val
        then eval expr
        else eval $ enot expr
...
forceForall tv vars e pos res = do
  qExpr@(Pos _ (Quantified Forall _ _ _ e')) <- evalQuantified (attachPos pos $ Quantified Forall tv vars [] e)  
  if res
...
exec stmt@(Pos pos s) = case s of
    ...
    Call _ lhss name args -> execCall name lhss args pos
...
execCallBySig sig lhss args pos = do
  ...
  where
    ...
    selectDef tc defs = do
      ...
      return (sig, defs !! i)
    dummyBlock = (singletonBlock . gen . Havoc . psigModifies) sig
    ...
    dummyDef tc = PDef {
        ...
        pdefBody = ([], startLabel dummyBlock, M.fromList $ toBasicBlocks dummyBlock),
        pdefPos = noPos
      }
...
execProcedure sig def args lhss callPos = let 
  ...
  execBody = do    
    ...
    pos <- exitPoint <$> execBlock (psigName sig) blocks (pdefStart def)
    checkPostonditions sig def pos    
    mapM (eval . attachPos (pdefPos def) . Var) outs
  in do
    argsV <- mapM eval args
    mem <- saveOld
    executeLocally (enterProcedure sig def args lhss) ins argsV (map itwWhere (psigRets sig ++ fst3 (pdefBody def))) execBody `finally` restoreOld mem       
...
--          else return $ MapUpdate m' args' new'   
      ...
      Quantified qop tv bv trggs e -> Quantified qop tv bv trggs <$> evalQuantified' (vars ++ map fst bv) e
...
checkMapConstraints r actuals pos = do
  ...
  where
    isGuarded (Pos _ (Quantified Lambda _ vars _ expr)) = case node expr of
    ...
    enableMapConstraint c val = case c of
      (Pos p (Quantified Lambda tv formals trggs body)) -> case node body of
        BinaryExpression Implies guard _ -> do
          let cond = if val then guard else enot guard
          enforceMapConstraint (Pos p (Quantified Lambda tv formals trggs cond)) actuals pos
          checkSatStep pos
...
enforceMapConstraint c actuals pos = 
  let
    Quantified Lambda tv formals _ body = node c
  ...
  in if isNothing $ unifier tv formalTypes actualTypes -- Is the constraint applicable to these types?
    then return ()
      ...
      Quantified Forall tv' vars _ e -> locally (enforceOnCache tv' vars e) -- parametrized forall constraint: check it on cached values
...
preprocess (Program decls) = do
  ...
  where
    processDecl decl = case node decl of
      ...
      ProcedureDecl _ name _ args rets _ (Just body) -> processProcedureBody name (position decl) (map noWhere args) (map noWhere rets) body
      ImplementationDecl _ name _ args rets bodies -> mapM_ (processProcedureBody name (position decl) args rets) bodies
      AxiomDecl _ expr -> extendNameConstraints conGlobals expr
      VarDecl _ vars -> mapM_ (extendNameConstraints conGlobals) (map itwWhere vars)      
      ConstantDecl _ True names t _ _ -> do
...
processFunction :: (Monad m,Functor m) => Id-> [Maybe Id] -> (Maybe Expression) -> [Attribute] -> SafeExecution m ()
...
processProcedureBody :: (Monad m,Functor m) => Id -> SourcePos -> [IdType] -> [IdType] -> Body -> SafeExecution m ()
processProcedureBody name pos args rets body = do
  ...
  let flatBody = (map (mapItwType (resolve tc)) (concatMap snd $ fst body), startLabel $ snd body, M.fromList (toBasicBlocks $ snd body))
  let allLocals = params ++ fst3 flatBody
...
extractConstraints' tv vars guards body = case (node body) of
  Quantified Forall tv' vars' _ bExpr -> extractConstraints' (tv ++ tv') (vars ++ vars') guards bExpr
  Quantified Exists _ _ _ _ -> M.empty -- ToDo: skolemize?
